 ...
 */
struct buf *
UNIX_bread(dev, blkno)
dev_t dev;
daddr_t blkno;
{
        register struct buf *bp;

        bp = UNIX_getblk(dev, blkno);
        ...
        bp->b_flags |= B_READ;
        bp->b_bcount = BSIZE;
        (*bdevsw[major(dev)].d_strategy)(bp); /*This will call the block strategy for the block device*/
#ifdef  DISKMON
        io_info.nread++;
#endif
        UNIX_iowait(bp);
...
daddr_t blkno, rablkno;
{
        ...
        if (rablkno && !UNIX_incore(dev, rablkno)) {
                rabp = UNIX_getblk(dev, rablkno);
...
register struct buf *bp;
{
        ...
        if (bp->b_flags&B_WANTED)
                UNIX_wakeup((caddr_t)bp);
        if (bfreelist.b_flags&B_WANTED) {
                bfreelist.b_flags &= ~B_WANTED;
                UNIX_wakeup((caddr_t)&bfreelist);
        }
    ...
    loop:
        ...
        for (bp=dp->b_forw; bp != dp; bp = bp->b_forw) {
                ...
                if (bp->b_flags&B_BUSY) {
                        bp->b_flags |= B_WANTED;
                        UNIX_sleep((caddr_t)bp, PRIBIO+1);
                        goto loop;
                }
                spl0();
#ifdef  DISKMON
        ...
        if (bfreelist.av_forw == &bfreelist) {
                bfreelist.b_flags |= B_WANTED;
                UNIX_sleep((caddr_t)&bfreelist, PRIBIO+1);
                goto loop;
        }
        spl0();
        UNIX_notavail(bp = bfreelist.av_forw);
 ...
 * swap I/O
 */
UNIX_swap(p,blkno, coreaddr, count, rdflg,uflag)
        struct  proc  *p;
{
        register struct buf *bp;
        register int c, pt, i;
        ...
        while (bp->b_flags&B_BUSY) {
                bp->b_flags |= B_WANTED;
                UNIX_sleep((caddr_t)bp, PSWP+1);
        }
        if (uflag) {    /* get current page table size */
                UNIX_ptaccess(p, Swap2map, swap2utl);
...
int (*strat)();
{
        ...
        if (UNIX_useracc(u.u_base,u.u_count,rw) == NULL) {
                u.u_error =EFAULT;
                return; }
        spl6();
        while (bp->b_flags&B_BUSY) {
