...
register struct proc *p;
{
        ...
        while (xswaplock & s) {
                xswapwant |= s;
                UNIX_sleep((caddr_t)map, PSWP);
        }
        xswaplock |= s;
        splx(a);
        if(os == 0)
                os = p->p_size;
        UNIX_ptaccess(p, map, utl);
        ...
        if ((p->p_flag & SSPART) == 0)  {
                a = UNIX_malloc(swapmap, ctod(p->p_size));
                if(a == NULL)
                        UNIX_panic("out of swap space");
                if (p->p_textp)
                        UNIX_xccdec(p->p_textp, p);
                p->p_swaddr = a;
                p->p_swsize = 0;
                for(i=0; i<ss; i++)
        ...
        if ( (os + UPAGES) == p->p_size) {
                if (ff >= 0)    /* do a complete swap */
                        UNIX_swap(p, a, p->p_tsize, p->p_size, B_WRITE, 1);
                else {
                        p->p_flag |= SSPART;    /* partial  swap */
                        i = p->p_tsize+(p->p_swsize ? p->p_swsize-UPAGES:0);
                        UNIX_swap(p, p->p_swaddr+p->p_swsize, i, -ff,
                ...
                UNIX_swap(p, a, p->p_tsize, os+UPAGES-ss, B_WRITE, 1);
                UNIX_swap(p, a+p->p_size-ss, p->p_tsize+os-ss, ss, B_WRITE, 0);
        }
        p->p_flag &= ~SLOAD;
        if (ff)  {
                if (ff > 0)
                        UNIX_memfree(utl+128*UPAGES+p->p_tsize, os);
                else
                        UNIX_memfree(utl+UPAGES*128+p->p_tsize+p->p_swsize
                            +ff-UPAGES*(1-uflag), -ff-uflag*UPAGES);
                if ((p->p_flag & SSPART) == 0)
                        UNIX_memfree(map, UPAGES+szpt);
        } else {
...
UNIX_xfree()
{
        ...
        if(--xp->x_count==0 && (ip->i_mode&ISVTX)==0) {
                xp->x_iptr = NULL;
                UNIX_mfree(swapmap, ctod(xp->x_size), xp->x_daddr);
                UNIX_memfree(((int *)&u) + UPAGES*128, u.u_tsize);
                ip->i_flag &= ~ITEXT;
                if (ip->i_flag&ILOCK)
                        ip->i_count--;
                else
...
register struct inode *ip;
{
        ...
        for (xp = &text[0]; xp < &text[NTEXT]; xp++) {
                ...
                if(xp->x_iptr == ip) {
                        ...
                        if (xp->x_ccount == 0)
                                ...
                                UNIX_ptaccess(xp->x_caddr, Xallmap, xallutl);
        ...
        if((xp=xp1) == NULL) {
                UNIX_printf("out of text");
                UNIX_psignal(u.u_procp, SIGKIL);
                return;
        }
        ...
        xp->x_size = ts;
        if((xp->x_daddr = UNIX_malloc(swapmap, ctod(ts))) == NULL)
                UNIX_panic("out of swap space");
        u.u_procp->p_textp = xp;
        UNIX_xexpand(xp);
        UNIX_chgprot(RW,0,0);
...
register  struct text *xp;
{
        ...
        if (UNIX_memall(((int *)&u) + UPAGES*128, btoc(u.u_exdata.ux_tsize)) != NULL) {
                xp->x_caddr = u.u_procp;
                if ((xp->x_flag&XLOAD)==0)
                        UNIX_swap(u.u_procp,xp->x_daddr,0, xp->x_size, B_READ,0);
                xp->x_ccount++;
                UNIX_chgprot(RO,0,0);
                UNIX_xunlock(xp);
                return;
...
register struct proc *p;
{
        ...
        if (--xp->x_ccount==0) {
                if (xp->x_flag&XWRIT) {
                        xp->x_flag &= ~XWRIT;
                        UNIX_swap(p,xp->x_daddr,0,xp->x_size,B_WRITE,0);
                }
                UNIX_ptaccess(p, Xccdmap, xccdutl);
                UNIX_memfree(xccdutl + UPAGES*128, p->p_tsize);
        }
        if (xp->x_caddr == p)
