...
UNIX_dzopen(d,flag)
{
        ...
        register dev ;
        extern UNIX_dzstart() , UNIX_dzscan() ;
        ...
        if (dz_timer == 0) {
                dz_timer++ ;
                UNIX_timeout(UNIX_dzscan,0,60) ;
                }
        tp = &dz_tty[dev] ;
//J     TYPE MISMATCH
//        tp->t_addr = ((struct dzregs *)DZADDR) + (dev>>3);
        tp->t_oproc = UNIX_dzstart ;
        tp->t_iproc = NULL;
        tp->t_state |= WOPEN ;
...
UNIX_dzioctl(dev,cmd,addr,flag)
caddr_t addr;
dev_t dev;
{
        register struct tty *tp;
        ...
        tp = &dz_tty[minor(dev)];
        if (UNIX_ttioccomm(cmd,tp,addr,dev)) {
                if (cmd==TIOCSETP || cmd==TIOCSETN)
                        UNIX_dzparam(minor(dev));
        } else
                u.u_error = ENOTTY;
}
...
register struct tty *tp ;
{
        ...
        if ((c = UNIX_getc(&tp->t_outq)) >= 0) {
                if (c >= 0200 && (tp->t_flags&RAW) == 0) {
                        dzaddr->dztcr &= ~unit ;
                        tp->t_state |= TIMEOUT ;
                        UNIX_timeout(UNIX_ttrstrt,tp,(c&0177)+6) ;
                        }
                else {
                        tp->t_char = c ;
                        tp->t_state |= BUSY ;
                ...
                if (tp->t_outq.c_cc <= TTLOWAT && tp->t_state&ASLEEP) {
                        tp->t_state &= ~ASLEEP ;
                        if (tp->t_chan)
                                UNIX_mcstart(tp->t_chan, (caddr_t)&tp->t_outq);
                        else UNIX_wakeup(&tp->t_outq) ;
                        }
                }
                splx(sps) ;
}
...
UNIX_dzscan()
{
        ...
        for (i = 0 ; i < dz_cnt ; i++) {
                ...
                UNIX_timeout(UNIX_dzscan,0,2*60) ;
}
