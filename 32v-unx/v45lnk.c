...
if (V11tab.bp == 0) /* grab system buffer */
        V11tab.bp = UNIX_geteblk() ;  /*  VAX call */
...
if (V11tab.bp) UNIX_brelse(V11tab.bp) ; /* release system buffer */
V11tab.bp = 0 ;
}
...
i = UNIX_uballoc(V11tab.bp->b_un.b_addr,BUFSIZ,1) ; /* get UBA map entry for
...
while ((u.u_count > 0) && (u.u_error == 0)) {
        UNIX_iomove(V11tab.bp->b_un.b_addr,UNIX_min(BUFSIZ,u.u_count),B_WRITE) ; /* move
        ...
        UNIX_sleep(LNKADR,V11PRI) ; /* wait for transfer to finish(interrupt) */
        spl0() ;
...
spl0() ;
i = UNIX_uballoc(V11tab.bp->b_un.b_addr,BUFSIZ,1) ;
j = i & 0x3ffff ; /* start map no. & byte offset */
...
while ((u.u_count>0) && (u.u_error == 0)) {
        ...
        UNIX_iomove(V11tab.bp->b_un.b_addr,UNIX_min(BUFSIZ,u.u_count),B_READ) ;
        }
