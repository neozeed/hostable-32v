...
int UNIX_uballoc(baddr,bcnt,bdpflg)
char  bdpflg ;
char * baddr ;
unsigned short bcnt;
{
...
spl6() ;
while ((regnum = UNIX_malloc(ubamap,nmreg) - 1) < 0) {
        /* wait for no. of mapping reg's requested */
        umrwant++ ;
        UNIX_sleep(ubamap,PSWP) ;
        }
if (bdpflg)  /* buffered data path BDP 1-15 */
        while ( (bdp=UNIX_malloc(bdpmap,1)) == NULL)
                {
                bdpwant++;
                UNIX_sleep(bdpmap, PSWP);
                }
else { /* BDP 0 = DDP */
        bdp = 0 ;
        }
...
/* free mapping reg's */
UNIX_mfree(ubamap,nmreg,regnum+1) ;
if (umrwant) {
        umrwant = 0;
        UNIX_wakeup(ubamap);
}
...
UNIX_ubainit()
{
        UNIX_mfree(ubamap, 496, 1);
        UNIX_mfree(bdpmap, 15, 1);
}
