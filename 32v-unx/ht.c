...
UNIX_htclose(dev, flag)
{
        register int unit;

        unit = minor(dev) & 03;
        if (flag == FWRITE || ((flag&FWRITE) && (h_flags[unit]&H_WRITTEN))) {
                UNIX_hcommand(dev, WEOF);
                UNIX_hcommand(dev, WEOF);
                UNIX_hcommand(dev, SREV);
        }
/*      hcommand(dev, REW);     */
/* for 'mtm' file positioning */
        if((minor(dev)&4) == 0) /* no 4 -> rewind */
                UNIX_hcommand(dev,REW) ;
        h_openf[unit] = 0;
}

UNIX_hcommand(dev, com)
{
...
UNIX_htintr(mbastat, as)
{
        ...
        if (HTADDR->htds&(ERR|EOT|TM) || mbastat & MBAEBITS) {
                ...
                if(HTADDR->htds&EOT || (HTADDR->htds&MOL)==0) {
                        ...
                        state = SOK;
                if(httab.b_errcnt > 4)
                        UNIX_deverror(bp, HTADDR->hter, mbastat);
...
{
        UNIX_htphys(dev);
        UNIX_physio(UNIX_htstrategy, &rhtbuf, dev, B_READ);
}

UNIX_htwrite(dev)
{
        UNIX_htphys(dev);
        UNIX_physio(UNIX_htstrategy, &rhtbuf, dev, B_WRITE);
}
