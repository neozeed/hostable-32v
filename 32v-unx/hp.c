...
register struct buf *bp;
{
        ...
        if (unit >= NHP ||
            bp->b_blkno < 0 ||
            (bn = UNIX_dkblock(bp))+sz > sizes[xunit&07].nblocks) {
                bp->b_flags |= B_ERROR;
                UNIX_iodone(bp);
                return; //another weird K&R Thing?
        }
        ...
        if(hptab.b_active) {
                ...
                if (hpaddr->hpds & ERR || mbastat & MBAEBITS) {         /* error bit */
                        ...
                        if(hptab.b_errcnt > 27)
                                UNIX_deverror(bp, mbastat, hpaddr->hper1);
                        if((bp->b_flags&B_PHYS) == 0 &&
                           (hpaddr->hper1 & (DCK|ECH)) == DCK) {
                                ...
                                hptab.b_active++;
                                UNIX_printf("%D ", bp->b_blkno);
                                UNIX_prdev("ECC", bp->b_dev);
                        }
...
UNIX_hpread(dev)
{
        ...
        UNIX_physio(UNIX_hpstrategy, &rhpbuf, dev, B_READ);
}

UNIX_hpwrite(dev)
{

        UNIX_physio(UNIX_hpstrategy, &rhpbuf, dev, B_WRITE);
}
