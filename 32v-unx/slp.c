...
printf("s");
         ...
         */
psig:
        UNIX_resume(u.u_procp->p_addr, u.u_qsav);
}
...
register struct proc *p;
{
        ...
        if (p->p_stat==0 || p->p_stat==SZOMB)
                UNIX_panic("Running a dead proc");
        p->p_wchan = 0;
        p->p_stat = SRUN;
        UNIX_setrq(p);
        if(p->p_pri < curpri)
...
UNIX_sched()
{
        ...
        iterate=0;
loop:
		/*Until we get anywhere near user processes, lets just kill ourselves..*/
        printf("\r%d",iterate);iterate++;
        if(iterate>1000)
        ...
        if (outage == -20000) {
                runout++;
                UNIX_sleep((caddr_t)&runout, PSWP);
                goto loop;
        }
        spl0();
        ...
        if (maxsize>=0 || (outage>=3 && inage>=2)) {
                ...
                UNIX_xswap(p,-memneed,0,-1);
                goto loop;
        }
        spl6();
        runin++;
        UNIX_sleep((caddr_t)&runin, PSWP);
        goto loop;
}
...
register struct proc *p;
{
        ...
        if ((p->p_flag & SSPART) == 0) {
                ...
                if (UNIX_memall(Swapmap,UPAGES + pt) == 0)
                        goto nomem;
                for (i=UPAGES+pt; --i>=0; ) {   /* map u-area & page tables */
                        Swapmap[i] |= PG_V + PG_KW;
                        UNIX_mtpr(TBIS, swaputl + (128 * i));
                }
                if(UNIX_memall(&swaputl[(UPAGES*128)+tsize-tmem],p->p_size-UPAGES+tmem)==0){
                                UNIX_memfree(Swapmap, UPAGES + pt);     /* free u-area & page tables */
                                goto nomem;
                }
                ...
                UNIX_ptaccess(p, Swapmap, swaputl);
                pt = ((struct user *)swaputl)->u_pcb.pcb_szpt;
                if (UNIX_memall(&swaputl[(UPAGES*128)+tsize-tmem], p->p_swsize-UPAGES
                                + tmem) == 0)
                        goto nomem;
                ip = &swaputl[(UPAGES*128)];
                for (i=tmem; --i>=0; )
        ...
        if (xp) {
                if (xp->x_ccount==0) {
                        ...
                        if ((xp->x_flag&XLOAD)==0)
                                UNIX_swap(p,xp->x_daddr,0,xp->x_size,B_READ,0);
                } else {
                        ...
                        if (xp->x_caddr == 0)
                                UNIX_panic("lost text");
                        UNIX_ptaccess(xp->x_caddr, Swap2map, swap2utl);
                        UNIX_bcopy(swap2utl + 128*UPAGES, swaputl + 128*UPAGES, 4*xp->x_size);
                }
                xp->x_ccount++;
                UNIX_xunlock(xp);
        }
        if ((p->p_flag & SSPART) == 0)
                UNIX_swap(p, x, tsize, p->p_size, B_READ, 1);
        else {
                UNIX_swap(p, x+UPAGES, tsize, p->p_swsize-UPAGES, B_READ, 0);
                p->p_flag &= ~SSPART;
        }
        ...
        UNIX_mfree(swapmap, ctod(p->p_size), x);
...
UNIX_swtch()
{
         ...
         */
        if (u.u_procp != &proc[0]) {
                if (UNIX_save(u.u_rsav)) {
                        return;
                }
                UNIX_resume(proc[0].p_addr, u.u_qsav);
        }
        /*
         ...
         */
        if (UNIX_save(u.u_qsav)==0 && UNIX_save(u.u_rsav))
                return;
loop:
        spl6();
        runrun = 0;
        ...
        n = p->p_flag&SSWAP;
        p->p_flag &= ~SSWAP;
        UNIX_resume(p->p_addr, n? u.u_ssav: u.u_rsav);
}
...
//                u.u_rdir->i_count++;
         ...
         * copy.
         */
        if (UNIX_procdup(rpp) == NULL) {
                rip->p_stat = SIDL;
                for(n=UPAGES; --n>=0; )
                        rpp->p_addr[n] = rip->p_addr[n];
                UNIX_xswap(rpp, 0, 0, -1);
                rip->p_stat = SRUN;
        }
        u.u_procp = rip;
        UNIX_setrq(rpp);
...
UNIX_expand(change,region)
{
        ...
        if (change == 0)
                return;
        p0 = UNIX_mfpr(P0BR) + 4 * UNIX_mfpr(P0LR);
        p1 = UNIX_mfpr(P1BR) + 4 * UNIX_mfpr(P1LR);

        /* see if we must add another page to the user page table */
        if ((new = (p0 - p1 + 4*change)/4) > 0) {
                new = UNIX_ptexpand((new + 127)/128);
        }
        if (region == P0BR) {
                base = (struct pt_entry *)UNIX_mfpr(P0BR);
                base += (p0 = UNIX_mfpr(P0LR)) + (change > 0 ? 0 : change);
        } else {
                base = (struct pt_entry *)UNIX_mfpr(P1BR);
                base += (p1 = UNIX_mfpr(P1LR)) - (change > 0 ? change : 0);
        }
        if (change > 0) {
                new = UNIX_memall(base,change);
        } else
                UNIX_memfree(base,-change);
        if (region == P0BR)  {
                UNIX_mtpr(P0LR,p0 + change);
                u.u_pcb.pcb_p0lr = p0 + change | 0x04000000;
        } else {
                UNIX_mtpr(P1LR,p1 - change);
